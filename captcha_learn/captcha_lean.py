...
print("the max of the captcha text length:", MAX_CAPTCHA)  # 验证码最长4字符; 我全部固定为4,可以不固定. 如果验证码长度小于4，用'_'补齐
...
w_d = tf.Variable(0.01 * tf.random_normal([8 * 32 * 40, 1024]))
b_d = tf.Variable(0.1 * tf.random_normal([1024]))

w_out = tf.Variable(0.01 * tf.random_normal([1024, MAX_CAPTCHA * CHAR_SET_LEN]))
b_out = tf.Variable(0.1 * tf.random_normal([MAX_CAPTCHA * CHAR_SET_LEN]))
...
predict_text_r = char_set[predict_text[0]] + char_set[predict_text[1]] + char_set[predict_text[2]]+ char_set[predict_text[3]]

print("right: {}  predict: {}".format(text, predict_text_r))
