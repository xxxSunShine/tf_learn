...
def preprocess(gray):
    ...
    # 高斯平滑
    gaussian = cv2.GaussianBlur(gray, (1, 9), 0, 0, cv2.BORDER_DEFAULT)
    # 中值滤波
...
def findPlateNumberRegion(img):
    region = []
    # 查找轮廓
    iamge ,contours, hierarchy = cv2.findContours(img, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for dirpath, dirnames, filenames in  os.walk(input_path):
        for filename in filenames:
            if (filename.endswith(".jpg")):
                ...
                if img is not None:
                    dct_img =detect(img)
                    cv2.imwrite(output_path+"/"+filename,dct_img)
                else:
